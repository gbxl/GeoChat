...
public class GeoChat extends Application {
	...
	public Client getClient() {
		if (client == null) {
			GeoChat.appendLog("MissCall",
					"The application doesn't run as a Client");
		}
		return client;
	}
	...
	public Host getHost() {
		if (host == null) {
			GeoChat.appendLog("MissCall",
					"The application doesn't run as a Host");
		}
		return host;
	}
	...
	private void setNbMaxChannelPerClient(int nbMaxChannelPerClient) {
		...
		ed.commit();
		appendLog("NBMAXCHANNELPERCLIENT", "NBMAXCHANNELPERCLIENT is set to "
				+ nbMaxChannelPerClient);
	}
	...
	public void createClient(Handler mRedrawHandler, String username,
			String ipAddress, int port) throws Exception {
	...
	public void createHost(String username, int port, int nbMaxClients,
			int nbMaxChannelPerClient, int maxSizeFile) throws Exception {
		try {
			host = new Host(username, nbMaxClients, nbMaxChannelPerClient,
					maxSizeFile, port);
			setUsername(username);
