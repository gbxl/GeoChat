...
public class HostSettings extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		nbChannelPerCli.setText(""
				+ GeoChat.getInstance().getNbMaxChannelPerClient());
		...
		btnConnect.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				try {
					GeoChat.getInstance().createHost(
							hostUsername.getText().toString(),
							Integer.parseInt(port.getText().toString()),
							Integer.parseInt(nbMaxCli.getText().toString()),
							Integer.parseInt(nbChannelPerCli.getText()
									.toString()),
							Integer.parseInt(sizeMaxFile.getText().toString()));
	...
	private void checkWifiState() {
		...
		if (!wifiManager.isWifiEnabled()) {
			...
			Toast.makeText(getApplicationContext(),
					"Enable your wifi conection please.", Toast.LENGTH_LONG)
					.show();
